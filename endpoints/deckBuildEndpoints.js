...
router.get('/delete/:id', async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        // var deckToDelete = await client.query(`SELECT * FROM decks where owner_id=${req.session.user.id} AND id='${req.params.id}'`);
        // if(deckToDelete.rows > 0) {
        //     console.log("DeckFound");
            await client.query(`DELETE FROM decks WHERE owner_id=${req.session.user.id} AND id='${req.params.id}'`);
        // } else {
        //     throw "Deck belongs to different user or doesn't exist"
        // }

        res.redirect('/deckBuild/decks');
    } catch(error) {
        res.redirect('/');
    }
})